...
RUN pip3 install ./src/swin && \
    : "Install the model" && \
    python3 -c "from transformers import AutoFeatureExtractor, SwinForImageClassification; AutoFeatureExtractor.from_pretrained('${MODEL_NAME}'); SwinForImageClassification.from_pretrained('${MODEL_NAME}')" && \
    colcon build
